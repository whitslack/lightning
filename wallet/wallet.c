...
void wallet_channel_save(struct wallet *w, struct channel *chan)
{
	...
	if (chan->scid)
		db_bind_short_channel_id_str(stmt, chan->scid);
	else
		db_bind_null(stmt);
	...
	db_bind_int(stmt, channel_has(chan, OPT_ANCHOR_OUTPUTS));
					  ...
					  const struct sha256 *local_offer_id)
{
	...
	if (payment->local_offer_id != NULL)
		db_bind_sha256(stmt, payment->local_offer_id);
	else
		db_bind_null(stmt);
					  ...
					  const struct sha256 *local_offer_id)
{
	...
	payment->local_offer_id = tal_dup_or_null(payment, struct sha256, local_offer_id);
					   ...
					   struct db_stmt *stmt)
{
	...
	payment = wallet_payment_new(ctx,
				     ...
				     take(db_col_optional(NULL, stmt, "local_offer_id", sha256)));
				 ...
				 int faildirection)
{
	...
	if (failchannel) {
		db_bind_short_channel_id_str(stmt, failchannel);
...
struct db_stmt *payments_by_offer(struct wallet *wallet,
				  const struct sha256 *local_offer_id)
{
	struct db_stmt *stmt;
	stmt = db_prepare_v2(wallet->db, SQL("SELECT"
